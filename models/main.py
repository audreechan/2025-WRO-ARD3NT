...
def detect_lines(image):
    ...
    # Blue/black mask (dark blues or near-black)
    lower_blue = np.array([90, 50, 0])
    upper_blue = np.array([130, 255, 80])
    blue_mask = cv2.inRange(hsv, lower_blue, upper_blue)

    # Combine masks
    mask = cv2.bitwise_or(orange_mask, blue_mask)
    ...
    if lines is not None:
        for line in lines:
            x1, y1, x2, y2 = line[0]
            # Adjust y-coordinates for ROI offset
            y1 += start_y
            y2 += start_y
